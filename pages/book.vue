      ...
      <div class="list" v-for="list in items" :key="list._id">
        ...
        <div class="book-des">
          ...
          <p class="book-info">
            ...
            <span class="book-state" :class="{'book-success': list.state === 1}">
              {{ list.state === 1 ? '待售' : '已售' }}
            </span>
          </p>
        </div>
...
export default {
  ...
  computed: {
    ...
    haveMore () {
      return this.$store.state.book.data.pagination.current_page === this.$store.state.book.data.pagination.total_page
    }
  },
