<template>
  <div class="article-list" :class="{ mobile: mobileLayout }">
    <div class="article-cont">
      ...
      <div class="meta">
        <span class="time">{{
          article.create_at | dateFormat('yyyy.MM.dd hh:mm')
        }}</span>
        <span v-if="!mobileLayout" class="num"
          >字数 {{ article.content.length }}</span
        >
    ...
    <div class="item">
      <div class="info">
        <div class="info-left">
          ...
          <span v-if="!mobileLayout" class="tag">
    ...
    <appreciate
      ...
      class="item appreciate-list"
    ></appreciate>

    <book :mobile-layout="mobileLayout" class="item book-list"></book>
    ...
    <div class="relative-article item">
      ...
      <div class="relative-list">
        <li v-for="item in relativeList" :key="item.key" class="relative-item">
          ...
          <p v-if="!mobileLayout" class="descript">
            {{
        ...
        <p v-if="relativeList.length === 0" class="empty">暂无推荐文章</p>
      </div>
    </div>

    <div class="comment">
      <comments v-if="article.title" :post-id="article.id"></comments>
    </div>

    <aside v-if="!mobileLayout">
      <div class="like" :class="{ 'is-liked': isLiked }" @click="like">
        <i class="iconfont icon-like like" :class="{ rubberBand: isLiked }"></i>
        <div class="like-decoration" :class="{ active: isLiked }"></div>
...
export default {
  ...
  computed: {
    ...
    toc() {
      ...
      const toc = markdown(this.article.content).toc;
      toc.forEach(item => {
  ...
  methods: {
    async like() {
      ...
      const res = await this.$store.dispatch('article/likeArt', {
