...
export const actions = {
  ...
  async likeComment ({ commit }, data) {
    ...
    if (res && res.code === 1) {
      commit('LIKE_ITEM', data)
    }
    return res
  },
  // 根据post-id获取评论列表
  loadCommentsByPostId ({ commit, state }, data) {
    ...
    setTimeout(async () => {
      ...
      if (res && res.code === 1) {
        let list
        if (res.result.pagination.current_page === 1) {
          list = res.result.data
        } else {
          list = [...state.data.data, ...res.result.data]
        }
