  ...
  <div id="share">
    <div class="share-box" :class="{ mobile: mobileLayout }">
      <a
        ...
        @click.prevent="
          shareWindow(
            `http://qr.topscan.com/api.php?text=${url}&w=300&el=h&m=10`
          )
        "
      >
      ...
      <a
        ...
        @click.prevent="
          shareWindow(
            `http://service.weibo.com/share/share.php?url=${url}&title=${title()}&source=${url}&sourceUrl=${url}&content=${description()}`
          )
        "
      >
      ...
      <a
        ...
        @click.prevent="
          shareWindow(
            `http://sns.qzone.qq.com/cgi-bin/qzshare/cgi_qzshare_onekey?url=${url}&title=${title()}&summary=${description()}&site=${url}`
          )
        "
      >
      ...
      <a
        ...
        @click.prevent="
          shareWindow(
            `https://www.douban.com/recommend/?url=${url}&title=${title()}`
          )
        "
      >
      ...
      <a
        ...
        @click.prevent="
          shareWindow(`http://twitter.com/share?text=${title()}&url=${url}`)
        "
      >
      ...
      <a
        ...
        @click.prevent="
          shareWindow(
            `http://www.linkedin.com/shareArticle?title=${title()}&summary=${description()}&mini=true&url=${url}`
          )
        "
      >
