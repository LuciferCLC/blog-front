<template>
  <div tag="div" class="aside">
    <div key="1" class="aside-item hot-article">
      <h3 class="title">
      ...
      <transition-group tag="div" class="hot-article-list" name="list">
        <p v-for="list in hotArt" :key="list._id" class="list">
          <nuxt-link :to="`/article/${list._id}`" :title="list.title">
    ...
    <div key="2" class="aside-item aside-tag">
      <h3 class="title">
        <span class="title-name">标签</span>
        <span class="line"></span>
      </h3>
      <ul class="tag clearfix">
        <li v-for="item in tag" :key="item.id" class="tag-item">
    ...
    <div key="5" v-fix class="aside-item aside-tag aside-fix">
      <ul class="tag clearfix">
        <li v-for="item in tag" :key="item.id" class="tag-item">
          <nuxt-link :to="`/tag/${item._id}`">
            {{ item.name }}
...
export default {
  ...
  data() {
    return {
      open: false,
      keyword: '',
      saying: ['当一个人成了谜', '我有一瓶酒', '去他妈的生活', '未寄出的信']
    };
  },
